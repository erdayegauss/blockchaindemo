...
exports.getMembers = function(req, res, next) {
    ...
    return businessNetworkConnection.connect(config.composer.adminCard)
        .then(() => {
            return businessNetworkConnection.getParticipantRegistry(config.composer.NS+'.'+req.body.registry)
...
/*
        businessNetworkConnection.revokeIdentity(config.composer.NS+'.'+req.body.type+'#'+req.body.id, req.body.id)
        .then((result) => {
...
exports.getAssets = function(req, res, next) {
    ...
    .then((bnd) => {
        ...
        return businessNetworkConnection.connect(config.composer.adminCard)
            .then(() => {
                return businessNetworkConnection.getAssetRegistry(config.composer.NS+'.'+req.body.registry)
                    .then(function(registry){
                        return registry.getAll()
                        .then ((members) => {
...
exports.addMember = function(req, res, next) {
    ...
    .then(() => {
        ...
        return businessNetworkConnection.getParticipantRegistry(config.composer.NS+'.'+req.body.type)
        .then(function(participantRegistry){
            ...
            .catch((_res) => {
                ...
                let participant = factory.newResource(config.composer.NS, req.body.type,req.body.id);
                participant.name = req.body.name;
                participantRegistry.add(participant)
                .then(() => {console.log(req.body.name+' successfully added'); res.send(req.body.name+' successfully added');})
                .catch((error) => {console.log(req.body.name+' add failed',error); res.send(error);});
            });
        })
